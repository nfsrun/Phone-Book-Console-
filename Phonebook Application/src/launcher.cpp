 ...
 *      Author: Kevin Tran
 *      launcher.cpp class
 *      Sets up the program to launch the phone book. It has private fields itself and multiple methods
 *      to run different functions that would be apparent in phone book operations. This interacts with
 *      the Contact class in which Contacts is used to keep objects of each different contact.
...
public:
	...
	string getName(){
		...
		cin.ignore();
		getline(cin, name);
		//name that was passed in will have a \r character that needs to be removed
		//to make string comparisons more accurate.
